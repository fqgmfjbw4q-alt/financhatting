...
def register():
    data = request.json
    if User.query.filter_by(username=data['username']).first(): return jsonify({'error': 'Alınmış'}), 400
    user = User(full_name=data.get('full_name'), username=data['username'], password=generate_password_hash(data['password']))
...
def check_session():
    if 'username' in session: return jsonify({'logged_in': True, 'username': session['username'], 'avatar': session.get('avatar', '👤')})
    return jsonify({'logged_in': False})

@app.route('/api/feed')
def get_feed():
    posts = Post.query.order_by(Post.timestamp.desc()).limit(50).all()
    return jsonify([{'id': p.id, 'user': p.username, 'content': p.content, 'timestamp': p.timestamp.strftime('%Y-%m-%d %H:%M')} for p in posts])
